...
public class OnlineGradientDescent implements IFuzzyNeuralNetwork {
	...
	private double learnNetwork(List<Pair> learningDataset) {
		...
		for (int j = 0; j < learningDataset.size(); j++) {
			...
			for (int i = 0; i < rules.length; i++) {
				...
				// antecedent learning
				rules[i].mfA.a = a1 + η * (o - z) * σ / Math.pow(sumw, 2) * (μB * μB) / Math.pow(μA + μB - μA * μB, 2)
						* b1 * μA * (1 - μA);

				rules[i].mfA.b = b1 + η * (o - z) * σ / Math.pow(sumw, 2) * (μB * μB) / Math.pow(μA + μB - μA * μB, 2)
						* (-(x - a1)) * μA * (1 - μA);

				rules[i].mfB.a = a2 + η * (o - z) * σ / Math.pow(sumw, 2) * (μA * μA) / Math.pow(μA + μB - μA * μB, 2)
						* b2 * μB * (1 - μB);

				rules[i].mfB.b = b2 + η * (o - z) * σ / Math.pow(sumw, 2) * (μA * μA) / Math.pow(μA + μB - μA * μB, 2)
						* (-(y - a2)) * μB * (1 - μB);
	...
	public void writeLearnedParams2File() {
		...
		String filename = "res/anfis/online/learnedParams.txt";
	...
	public void writeRelativeError2File(List<Pair> learningDataset) {
		...
		String filename = "res/anfis/online/relError.txt";
	...
	public void writeEpochError2File(List<Pair> learningDataset, int epoch) {
		...
		String filename = "res/anfis/online/online_epochError.txt";
	...
	public void writeEpochErrorForEta2File(double η, List<Pair> learningDataset, int epoch) {
		...
		String filename = "res/anfis/online/XepochEtaError.txt";
