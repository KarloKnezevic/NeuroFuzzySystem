package hr.fer.zemris.neurofuzzysystem.expert.main;
...
import hr.fer.zemris.neurofuzzysystem.expert.conclusionEngine.*;
import hr.fer.zemris.neurofuzzysystem.expert.database.*;
import hr.fer.zemris.neurofuzzysystem.expert.input.*;
import hr.fer.zemris.neurofuzzysystem.expert.norm.*;
import hr.fer.zemris.neurofuzzysystem.expert.rule.*;
import hr.fer.zemris.neurofuzzysystem.expert.membershipFunctions.*;

/**
 * 
 * @author Karlo Knežević 1. laboratorijska vježba iz predmeta: Neizrazito
 *         evolucijsko i neuro računarstvo Fuzzy Boat
 *
 */
public class FuzzyControlSystem {

	/**
	 * Definirati ulaz i izlaz senzora. Definirati bazu pravila i popuniti je
	 * pravilima. Definirati ulaze i domenu mjerenih podataka. Pokrenuti stroj
	 * za zaključivanje. Algoritam: dok god ima rezultata sa senzora: čitaj
	 * rezultate izvedi zaključak reagiraj
	 * 
	 * @param args
	 */
	public static void main(String[] args) {
		...
		IInput sensor = new Sensor(new BufferedReader(new InputStreamReader(System.in)),
		...
		// ovo je ujedno i UNIVERZALAN SKUP - stvarna fizička domena u kojoj
		// elementi poprimaju numeričke vrijednosti
		...
		// korišteni operatori i norme -> lijepo se ponašaju Zadeh, Algebarski,
		// DRASTIC (NAJBOLJI!)
		...
		// definirana pravila; antecedens -> konsekvens; konsekvens mora biti
		// funkcija jer je dekomponiran (ne mora nužno biti funkcija, ali u tom
		// slučaju treba definirati inverz normi
		// u pravilima su navedeni modeli neizrazitih skupova kojima su
		// pridružene jezične varijable
		IRule[] rules = {
				// ako je udaljenost od lijeve strane mala i brzina oko 20,
				// skreni u desno do 10 stupnjeva
				new Rule(I.x(new Lfunc(L, 10, 30), new Lambda(V, 10, 20, 30)), new Lfunc(K, -10, 0)),
				// ako je udaljenost od desne strane mala i brzina oko 20,
				// skreni u lijevo do 10 stupnjeva
				...
				// ako je LK malen i brzina mala ili je DK malen i brzina mala,
				// povećaj akceleraciju
				...
				// ako V nije velika i LK i DK nisu mali, više ili manje povećaj
				// akceleraciju
				new Rule(I.x(NE.x(new Gamma(V, 40, 60)), NE.x(ILI.x(new Lfunc(LK, 30, 70), new Lfunc(DK, 30, 70)))),
						MANJEiliVISE.x(new Gamma(A, 0, 20))),
				// ako je V relativno velika i LK ili DK jako mali, jako koči
				new Rule(
						I.x(MANJEiliVISE.x(new Gamma(V, 45, 70)),
								ILI.x(VRLO.x(new Lfunc(LK, 50, 100)), VRLO.x(new Lfunc(DK, 50, 100)))),
						VRLO.x(new Lfunc(A, -35, -10))),
		...
		// sustav za zaključivanje: baza podataka, defazifikator, različiti
		// antecedensi
